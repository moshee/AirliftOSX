...
+ (void)deleteUploadAtURL:(NSString *)urlToDelete {
  ...
  NSMutableURLRequest *request = [ALUploadManager
      constructRequestToPath:[@"/" stringByAppendingString:hash]];
  if (request == nil) {
    return;
  }
  ...
  completionHandler = ^(NSData *data, NSURLResponse *response, NSError *error) {
    ...
    if (error != nil) {
      ...
      if ([httpResponse statusCode] == 204) {
        ...
        if (error != nil) {
          subtitle =
              [NSString stringWithFormat:@"Failed to parse server response "
                                         @"(server returned status: %ld)",
                                         [httpResponse statusCode]];
        } else {
          subtitle =
              [NSString stringWithFormat:@"Server returned error: %@",
  ...
  if (errMsg != nil) {
    ...
    [appDelegate
        showNotificationOfType:ALNotificationUploadError
                         title:[NSString stringWithFormat:@"Error copying %@",
                                                          linkableURL]
                      subtitle:subtitle
                additionalInfo:nil];
  } else {
...
- (void)URLSession:(NSURLSession *)session
  ...
  if (error != nil) {
    [appDelegate
        showNotificationOfType:ALNotificationUploadError
                         title:@"Error uploading"
                      subtitle:[NSString stringWithFormat:@"Failed to "
                                                          @"decode server "
                                                          @"response "
                                                          @"(status %d)",
                                                          responseCode]
