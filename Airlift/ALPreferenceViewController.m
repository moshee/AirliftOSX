...
+ (void)updatePassword:(NSString *)newPassword forHost:(NSString *)host {
  ...
  OSStatus status =
      SecKeychainFindGenericPassword(NULL, serviceLength, serviceChars,
                                     hostLength, hostChars, NULL, NULL, &item);

  if (status == 0) {
    status =
        SecKeychainItemModifyContent(item, NULL, passwordLength, passwordChars);
  } else {
  ...
  if (status != 0) {
    NSRunAlertPanel(@"Keychain error", @"Error updating password: %@", @"OK",
                    nil, nil, [self messageForStatusCode:status]);
  }
}
...
+ (NSString *)retrievePasswordForHost:(NSString *)host {
  ...
  OSStatus status = SecKeychainFindGenericPassword(
      NULL, serviceLength, serviceChars, hostLength, hostChars, &passwordLength,
      &passwordData, NULL);
  NSString *password = @"";
  ...
  case errSecItemNotFound:
    break;
  default:
    NSRunAlertPanel(@"Keychain error", @"Error retrieving keychain item: %@",
