...
- (IBAction) didEnterHostname:(id)sender {
	...
	// TODO: look up the corresponding password and set it if it's not empty? this could get messy though as people change stuff and then lots of incomplete hosts get associated with lots of incomplete passwords
...
+ (void) updatePassword:(NSString*)newPassword forHost:(NSString*)host {
	...
	const char* serviceChars   = [serviceName cStringUsingEncoding:NSUTF8StringEncoding];
	UInt32      passwordLength = (UInt32)newPassword.length;
	const char* passwordChars  = [newPassword cStringUsingEncoding:NSUTF8StringEncoding];
	
	SecKeychainItemRef item;
	
	OSStatus status = SecKeychainFindGenericPassword(NULL, serviceLength, serviceChars, hostLength, hostChars, NULL, NULL, &item);
	
	if (status == 0) {
		status = SecKeychainItemModifyContent(item, NULL, passwordLength, passwordChars);
	} else {
		status = SecKeychainAddGenericPassword(NULL, serviceLength, serviceChars, hostLength, hostChars, passwordLength, passwordChars, &item);
	}
...
+ (NSString*) retrievePasswordForHost:(NSString*)host {
	...
	OSStatus status = SecKeychainFindGenericPassword(NULL,
													 serviceLength, serviceChars,
													 hostLength, hostChars,
